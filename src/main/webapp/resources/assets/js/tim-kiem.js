...
const overlay_load = document.querySelector(".overlay");

addLoading = () => {
	loader.style.display = 'block';
	overlay_load.style.display = 'block';
}

removeLoading = () => {
	loader.style.display = 'none';
	overlay_load.style.display = 'none';
}
...
$("#btnApDung").click((e) => {
    page = 1;
    timKiem("http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham?page=" + page, false);
    //	e.preventDefault();
});



async function timKiem(callUrl, isPageCall) {
	addLoading()
    ...
    await $.get(url, function (data) {
        ...
        if (isPageCall == true) {
            optionPage(data, callUrl, page, flagPageBefore);
        }
        else {
            optionSearch(data, callUrl, page, flagPageBefore);
        }
    });
	removeLoading();
}
...
function redirectNext() {
    ...
    if (PAGE_PATH.includes("LSP") && !PAGE_PATH.includes("TH")) {
        ...
        flagPageBefore = page;
        page++;
        timKiem(`http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham/${maLoai}?page=` + page, true);
    }
        ...
        flagPageBefore = page;
        page++;
        timKiem(`http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham/${maTH}?page=` + page, true);
    }
        ...
        flagPageBefore = page;
        page++;
        timKiem(`http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham/${maDong}?page=` + page, true);
    }
        ...
        flagPageBefore = page;
        page++;
        timKiem(`http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham/${maLoai}/${maTH}?page=` + page, true);
    }
        ...
        flagPageBefore = page;
        page++;
        timKiem('http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham/tim-kiem-san-pham?tenSanPham=' + SearchTenSanPham + '&page=' + page, true);
    }
    else {

        flagPageBefore = page;
        page++;
        timKiem("http://localhost:8080/WebsiteBanNhacCu/danh-sach-san-pham?page=" + page, true);
    }
}
...
function optionPage(data, callUrl) {
    if (data.length > 0) {
        optionSearch(data, callUrl, page, flagPageBefore);
    }
    else {
...
function optionSearch(data, callUrl, page, flagPageBefore) {
    if (data.length <= 0) {
        console.log("page " + page + "=====bfire: " + flagPageBefore);
        page = flagPageBefore;
