...
<link rel="stylesheet"
	href="<c:url value="/static/assets/css/ProductDetail/ProductDetail.css"/>" />
<link rel="stylesheet"
	href="<c:url value="/static/assets/css/main.css"/>" />
<link rel="stylesheet"
	href="<c:url value="/static/assets/css/ProductDetail/Addition.css"/>" />
...
<body>
		...
		<div class="container body body_product">
			<div class="row main_info">
				...
				<div class="col-4 slider">
							...
							<c:forEach items="${ mauSanPhamDTOs }" var="mauSanPhamDTO">
								<div class="slider__image__frame">
									<img
										...
										class="slider__image__frame--detail" alt="" />
								</div>
							</c:forEach>
					...
					<i class="fas fa-chevron-right move arrow--right"></i> <i
						class="fas fa-chevron-left move arrow--left"></i>
				</div>

				<div class="col title__product">
					<div class="title">
						<span class="name__product">${ sanPhamDTO.tenSanPham }</span>
						<span class="id--sanpham">Mã sản phẩm: </span><span
							class="id--sanpham" id='maSP'>${ sanPhamDTO.id }</span> <span>Thương
							hiệu: <b class="brand">${ sanPhamDTO.tenThuongHieu }</b>
						</span>
						...
						<span>Giá bán: <b class="price__product"><fmt:formatNumber
									type="number" value="${ sanPhamDTO.giaBan }" /> VNĐ</b></span>
						<div class="select_color">
							...
							<c:forEach items="${ mauSanPhamDTOs }" var="mauSanPhamDTO">
								<c:if
									test="${ mauSanPhamDTOs.get(0).tenMau !=  mauSanPhamDTO.tenMau }">
									<div class="color">
					...
					<div class="interactive__general--info">
						<div class="interactive">
							<a href="">
								...
								<button class="button--inter themvao--giohang"
	...
	<div class="container info__specific">
		<div class="info">
			...
			<button class="btn btn-outline-danger" id="readmore"
				onclick="moreInfo()">
	...
	<h5 class="container title_product_evaluate">${ binhLuans.size() }
		Đánh giá ${ sanPhamDTO.tenSanPham }</h5>
	<div class="container evaluate__sendratting">
		<div class="evaluate">
			<div class="block__star">
				<div class="average__star">
					<h4>Sao trung bình</h4>
					<p class="figure__average"></p>
				</div>
				...
				<div class="user__ratting">
					<div class="user__ratting--sub">
						<span>5<span class="fa fa-star checked"></span></span>
						<div class="ratting rat"></div>
						<div class="ratting"></div>
						<div class="evaluate--adjust">
							<span class="amount_of_ratting_star">${ namSao }</span> <span
								class="amount_evaluate"> đánh giá</span>
						</div>
					</div>

					<div class="user__ratting--sub">
						<span>4<span class="fa fa-star checked"></span></span>
						<div class="ratting rat"></div>
						<div class="ratting"></div>
						<div class="evaluate--adjust">
							<span class="amount_of_ratting_star">${ bonSao }</span> <span
								class="amount_evaluate"> đánh giá</span>
						</div>
					</div>

					<div class="user__ratting--sub">
						<span>3<span class="fa fa-star checked"></span></span>
						<div class="ratting rat"></div>
						<div class="ratting"></div>
						<div class="evaluate--adjust">
							<span class="amount_of_ratting_star">${ baSao }</span> <span
								class="amount_evaluate"> đánh giá</span>
						</div>
					</div>

					<div class="user__ratting--sub">
						<span>2<span class="fa fa-star checked"></span></span>
						<div class="ratting rat"></div>
						<div class="ratting"></div>
						<div class="evaluate--adjust">
							<span class="amount_of_ratting_star">${ haiSao }</span> <span
								class="amount_evaluate"> đánh giá</span>
						</div>
					</div>

					<div class="user__ratting--sub">
						<span>1<span class="fa fa-star checked"></span></span>
						<div class="ratting rat"></div>
						<div class="ratting"></div>
						<div class="evaluate--adjust">
							<span class="amount_of_ratting_star">${ motSao }</span> <span
								class="amount_evaluate"> đánh giá</span>
						</div>
					</div>
				</div>
				...
				<div class="button_evaluate" hidden>
					<button class="btn btn-danger sendEvaluate">Gửi đánh giá
						của bạn</button>
				</div>
			</div>
		</div>
	...
	<div class="cmt__evaluate container">
		<c:forEach items="${ binhLuans }" var="binhLuan">
			<div class="user__cmt">
				<p>
					<b>${ binhLuan.nguoiDung.hoTen }</b><span> || Ngày <fmt:formatDate
							pattern="dd-MM-yyyy" value="${ binhLuan.ngayBinhLuan }" /></span>
				</p>
				<p>
					<c:forEach begin="1" end="5" varStatus="loop">
						<span
							class="fa fa-star <c:if test="${loop.index <= binhLuan.danhGia}">checked</c:if>"></span>
	...
	<button class="btn btn-outline-danger btn-morecmt"
		onclick="getMoreComment()">
		Xem thêm đánh giá&nbsp;<i class="fas fa-arrow-right"></i>
	</button>

	<script
		src="<c:url value="/static/assets/js/ProductDetail_page/UserRatting.js"/>"></script>
	<script
		src="<c:url value="/static/assets/js/ProductDetail_page/ProductDetail.js"/>"></script>
	<script
		src="<c:url value="/static/assets/js/ProductDetail_page/Comment.js"/>"></script>
